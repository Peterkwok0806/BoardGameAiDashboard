...
/// <summary>
/// FluentValidation rules for <see cref="BatchPredictCommand"/>.
/// </summary>
public sealed class BatchPredictCommandValidator : AbstractValidator<BatchPredictCommand>
{
    public BatchPredictCommandValidator()
    {
        RuleFor(x => x.Inputs)
            .NotNull().WithMessage("Inputs collection is required.")
            .NotEmpty().WithMessage("At least one input is required for batch prediction.")
            .Must(inputs => inputs.Count <= BatchPredictCommand.MaxBatchSize)
            .WithMessage($"Batch size cannot exceed {BatchPredictCommand.MaxBatchSize} predictions.");

        RuleForEach(x => x.Inputs)
            .SetValidator(new BatchPredictInputValidator());
    }
}

/// <summary>
/// FluentValidation rules for individual inputs in a batch predict command.
/// </summary>
public sealed class BatchPredictInputValidator : AbstractValidator<GameStatePredictionInput>
{
    public BatchPredictInputValidator()
    {
